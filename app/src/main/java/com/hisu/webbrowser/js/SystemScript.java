...
public class SystemScript {
	...
	public void setGlobalVar(String key, String value) {
		...
		try {
			...
			sp.edit().putString(MEMEORY_STORE_KEY, localJson.toString())
					.commit();
			Log.d(TAG, CommonFunction._FUNC_() + " key= " + key + " value ="
					+ value);
	...
	public String getGlobalVar(String key) {
		Log.d(TAG, " " + CommonFunction._FUNC_() + " called.");
		String ret = "";
		String localStr = getSharePreferences(mContext).getString(
				MEMEORY_STORE_KEY, "{}");
		try {
			JSONObject localJson = new JSONObject(localStr);
	...
	public void removeLocalVar(String key) {
		...
		try {
			...
			if (localJson.has(key)) {
				localJson.remove(key);
				sp.edit().putString(LOCAL_STORE_KEY, localJson.toString())
						.commit();
			}
		} catch (JSONException e) {
			// TODO Auto-generated catch block
	...
	public int toast(String message) {
		Log.d(TAG, " " + CommonFunction._FUNC_() + " called. message = "
				+ message);
		if (null == mHandler) {
			return -1;
		}
	...
	// launcherֱ��
	@JavascriptInterface
	public void sendBroadcastForAction(String action,String argument,String data) {
		...
		try {
			JSONArray paramsArr = new JSONArray(argument);
			for (int i = 0; i < paramsArr.length(); i++) {
				JSONObject arg = paramsArr.getJSONObject(i);
				Log.d("SystemScripe", "args[" + i + "]===" + arg.toString());
				if (!arg.has("key") || !arg.has("value")) {
					Toast.makeText(mContext, "参数错误，请检查", 1).show();
				} else if (arg.has("type")) {
					String key = arg.getString("key");
					String type = arg.getString("type").toLowerCase();
					if ("int".equals(type)) {
					...
					if("ctrlType".equals(arg.getString("key"))){
						intent.putExtra(arg.getString("key"),arg.getInt("value"));
					}else{
						intent.putExtra(arg.getString("key"),
								arg.getString("value"));
					}
	...
	public void openVLC(String path) {
		...
		// "rtsp://61.185.20.133:554/pag://10.63.32.53:7302:00000000001310010176:0:SUB:TCP?cnid=100001&pnid=1&auth=0");
	...
	public int openApp(String packageName, String activityName, String arguments) {
		...
		if (TextUtils.isEmpty(activityName)) {
			...
			ComponentName localComponentName = new ComponentName(packageName,
					activityName);
			intent = new Intent();
		...
		if (intent != null) {
			...
			if (!TextUtils.isEmpty(arguments)) {
				try {
					...
					for (int i = 0; i < paramsArr.length(); i++) {
						...
						if (!arg.has("key") || !arg.has("value")) {
							Toast.makeText(mContext, "参数错误，请检查",
						...
						if (!arg.has("type")) {
							...
							String key = arg.getString("key"), type = arg
					...
					// TODO Auto-generated catch block
					Toast.makeText(mContext, "参数错误，请检查", Toast.LENGTH_LONG)
							.show();
					e.printStackTrace();
				}
			}
			try {
				...
				Log.d(TAG,
						"SW_OPEN_APP ============ActivityNotFoundException========");
				return -2;
			}
		} else {
	...
	public void openUdiskVideo() {
		String path = new DataUtil(mContext).getsharepreferences("u_path")
				+ "/hisu";
		Log.e("path", path);
	...
	public int uninstallApp(String packageName) {
		try {
			...
			if (jsonObj.has(packageName)) {
				...
				sp.edit()
						.putString(Constants.APK_FILE_PATH_INFO,
								jsonObj.toString()).commit();
			}
			;
	...
	@JavascriptInterface
	public int openAppForAction(String Action, String arguments,String data) {
		Log.e(Action, arguments);
...
//			intent.setAction(Action);
        ...
        //ָ��Ҫ���ҵ�Activity Action:com.android.phone.action.TOUCH_DIALER
        //��ϵͳ�в�ѯָ����Activity Action
	...
	@JavascriptInterface
	public String getSysInfo(String attrstr) {

		Class<?> caPro = null;
		try {
			caPro = Class.forName("android.os.SystemProperties");
			Method method = caPro.getMethod("get", String.class);
			String sysInfo = (String) method.invoke(null, attrstr);
			return sysInfo;
	...
	public String getCaCardAddress() {
		...
		try {
			...
			Method method = caPro.getMethod("get", String.class);
			String caAddr = (String) method.invoke(null, "persist.sys.mmcp.smartcard");
			return caAddr;
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		} catch (NoSuchMethodException e) {
	...
	public void preventDefault(int keyCode) {
		...
		if (irkeyCode > 0)
			preventDefaultKeyList.add(irkeyCode);
	}
}
