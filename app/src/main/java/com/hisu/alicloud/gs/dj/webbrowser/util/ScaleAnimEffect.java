...
public class ScaleAnimEffect {
	...
	public Animation translAnimation(float fromXDelta, float toXDelta,
		...
		localTranslAnimation.setDuration(200L);
		localTranslAnimation.setFillAfter(true);
		localTranslAnimation.setInterpolator(new DecelerateInterpolator());//new DecelerateInterpolator(0.2f)减�?�AccelerateDecelerateInterpolator中间加�??
	...
	public Animation ScaleAnimation(float fromXScale,float toXScale,float fromYScale,float toYScale){
		ScaleAnimation localScaleAnimation = new ScaleAnimation(fromXScale, toXScale, fromYScale, toYScale,
				1, 0.5F, 1, 0.5f);
		localScaleAnimation.setFillAfter(true);//设置动画后是否保存状�?
		AccelerateInterpolator localAccelerateInterpolator = new AccelerateInterpolator();
		localScaleAnimation.setInterpolator(localAccelerateInterpolator);//设置动画启动后几秒开�?
		localScaleAnimation.setDuration(200L);
		return localScaleAnimation;
	}
	public Animation ScaleAnimation(float fromXScale,float toXScale,float fromYScale,float toYScale,boolean FillAfter){
		ScaleAnimation localScaleAnimation = new ScaleAnimation(fromXScale, toXScale, fromYScale, toYScale,
				1, 0.5F, 1, 0.5f);
		localScaleAnimation.setFillAfter(FillAfter);//设置动画后是否保存状�?
		AccelerateInterpolator localAccelerateInterpolator = new AccelerateInterpolator();
		localScaleAnimation.setInterpolator(localAccelerateInterpolator);//设置动画启动后几秒开�?
		localScaleAnimation.setDuration(200L);
		return localScaleAnimation;
